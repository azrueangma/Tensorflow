...
class GAN(object):
    ...
    def discriminator(self, x, keep_prob, d_name, reuse=False):
    ...
    def build_model(self, batch_size, noise_dim):
        ...
        D_real_logits= self.discriminator(self.X, self.keep_prob, 'Discriminator', reuse=False)
        G = self.generator(self.Z, 'Generator', reuse=False)
        D_fake_logits = self.discriminator(G, self.keep_prob, 'Discriminator', reuse=True)
        ...
        with tf.variable_scope("Optimization"):
            ...
            self.g_optim = tf.train.AdamOptimizer(self.learning_rate).minimize(self.g_loss, var_list=g_vars)
    ...
    def train(self, total_epoch, learning_rate, save_dir):
        ...
        for epoch in range(total_epoch):
            for step in range(total_step):
                ...
                _,  d_loss = self.sess.run([self.d_optim, self.d_loss], feed_dict={self.X: batch_images, self.Z: batch_z, self.learning_rate:u, self.keep_prob:0.7})
                _,  g_loss = self.sess.run([self.g_optim, self.g_loss], feed_dict={self.Z: batch_z, self.learning_rate:u, self.keep_prob:0.7})
                if step%10==0:
